...
#Get Only Genre Details 
songs_genre<- songs_top100|>
  select(track_artist, genre)

#creating Dataset to move forward with 
officalbardata<- bardata|>
  left_join(songs_genre, by = join_by(track_artist))
...
county_map <- maps::map("country", regions = "CAN"
                    , plot = FALSE, fill = TRUE) |>
st_as_sf()

view(country_map)

maps:map("country", region)
names(map)


maps::map("world", "canada", plot= FALSE, fill= FALSE)


# Get Canada provinces
canada_provinces <- gadm("CAN", level = 1, path = tempdir())

# Plot it
plot(canada_provinces["NAME_1"])


# Get Canada at country level
canada <- ne_countries(country = "Canada", returnclass = "sf")

# Or get provinces (if available)
states <- ne_states(country = "Canada", returnclass = "sf")|>
  st_as_sf()
states
