...
textspacesallocation<- data.frame(track_artist=NA, sentences= NA, containtextartist=NA)
...
for (a in artistvec){
 spotify_text_identify<-  songs_text|>
  mutate(containtextartist = str_extract(sentences, a))|>
   filter(!is.na(containtextartist)) #Used to remove NAs and cut down iteration computation time
...
textspacesallocation1<- textspacesallocation|>
  mutate( track_artist1 = case_when (
    track_artist == containtextartist ~ "x",
    TRUE ~ track_artist
  ))

#Get rid of artist mentions of themselves and thn removing the column because it does not add any useful information (it just reopeats the artists again), also dont need the wikipedia text anymore once I have the artist they have collabed with
textspacesallocation1<- textspacesallocation1|>
    filter(!track_artist1== "x")
Offical_artist_collabset<- textspacesallocation1|>
...
Offical_artist_collabset
songs_top100

#Plan Out The Network Graph:
#1. Artist Collab
#2. Artist Genre 
#3. Number of times artist mentioned (Not Network, just bar Chart)
#3. Number of times artist mentioned with Genre
#4. Length of music, maybe something like connection of 


##Artist Collab: (Using Offical_artist_collabset data set)
...
##All of Graphs Needed: (Using songs_top100) 
# Create the Nodes and Edges for a visble Graph 


##1. Artist Genre 
artistcollab_network<- as_tbl_graph(x= Offical_artist_collabset, nodes=track_artist, edges = containtextartist, directed= TRUE) #Not Mutually Directed 
gorder(artistcollab_network) #Number of Nodes 
gsize(artistcollab_network) # Number of Edges 



##2. How many Times an artist is mentioned in relation to Genre 
#Need Wrangling First 
#Need to Add Weight 

numberHits<- songs_top100|>
  select(track_artist, genre)|>
  group_by(track_artist, genre)|>
  summarize(Mentions= n())

NumberOfArtisitHits <- as_tbl_graph(x= numberHits, nodes=genre, edges =  track_artist, directed= FALSE) #Mutually Connected Each Artist Should have a genre (Check that definition)
gorder(NumberOfArtisitHits) #Number of Nodes 
gsize(NumberOfArtisitHits) # Number of Edges 


degreeArtistHits <- NumberOfArtisitHits |>
    activate(nodes) |>
    mutate(degree = centrality_degree(),
           strength = centrality_degree(weights = Mentions)) #Grabs how many times the artist has had a hot song for each genre




##3. Number of times artist mentioned 
# Need Wrangling First 
numberHits1<- songs_top100|>
  select(track_artist)|>
...
#4. Length of music, maybe something like connection -- Get Opinions ? Ask them What the think about the need for more.
artistcollab_network<- as_tbl_graph(x= Offical_artist_collabset, nodes=track_artist, edges = containtextartist, directed= FALSE)
gorder(artistcollab_network) #Number of Nodes 
gsize(artistcollab_network) # Number of Edges 
...
songs_top100_HomeTown<- songs_top100_urls|>
  

<th scope="row" class="infobox-label">Home town</th>
