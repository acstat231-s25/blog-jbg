...
#Grouping and counting how many artist are in each state (going to use as weight cues later) and delete everything after comma to match with the spatial data 
spotifydataHTLocations<- spotifydataHT|>
                         mutate(hometown= str_to_lower(hometown))|>
                         group_by(hometown)|>
                         summarise(count = n())

spotifydataHTLocations<- spotifydataHTLocations|>
  mutate(hometown = str_replace(hometown, ",.*", ""))

##States
# Create sf object for US states
state_map <- maps::map("state", regions = "."
                    , plot = FALSE, fill = TRUE) |>
st_as_sf()

#renaming ID to hometown so they merge 
state_map <- rename(state_map, hometown = ID)

#Combining
#Note this excludes those outside of US (There are only a few and we can mention the map visual)
fullStateSpotify<- spotifydataHTLocations|>
  inner_join(state_map,  by = join_by("hometown"))

#Check Imagine 
ggplot() +
  geom_sf(data=fullStateSpotify, aes(geometry=geom)
          , fill = "lightgrey", color = "white") +
  geom_sf(data=state_map, aes(geometry=geom)
          , fill = NA, color = "black") +
  theme_void()
...
#Tab 1: USA Hometowns
#Tab 2: Canada Hometowns
#Tab 3: South America hometowns? 
...
#Get the data for leaflet
#get link from phone 
# usa_sf_leaflet <- sf::read_sf("https://rstudio.github.io/leaflet/json/us-states.geojson")
# class(usa_sf_leaflet)
# names(usa_sf_leaflet)

# #changing column to hometown to combine data 
# usa_states_data_geom<- usa_sf_leaflet|>
#    select(-density)|>
#    rename(hometown = name)|>
#    mutate(hometown= str_to_lower(hometown))
# #Combine Spotify Data with USA
# ArtistUSA<- usa_states_data_geom|>
#   left_join(spotifydataHT, by= (join_by("hometown")))|>
#   drop_na()
...
#Collapsing track names into each state 
USAartistComb <- spotifydataHT|>
  tibble(
  category = hometown,
  text = track_artist)|>
  group_by(category)|>
...
#renaming for later dataset join 
USAartistC<- USAartistComb|>
  rename(hometown = category,
         track_artist = text)
#tibble
...
#Combine Spotify Data with USA
ArtistUSA<- usa|>
  left_join(USAartistC, by= (join_by("hometown")))|>
  drop_na() #removes artist in the list who do not live in the United states 

OfficalArtistUSA<- ArtistUSA|>
  mutate(Weight= str_count(track_artist, ",")+ 1)

 


leaflet(OfficalArtistUSA)|>
  addProviderTiles("CartonDB.DarkMatter")|>
  addPolygons(
    fillColor= "darkgreen",
    fillOpacity = "0.5",
    color="black",
...
ggplot() +
  geom_sf(data = canada_sf, aes(fill = GID_1), alpha = .5) +
  geom_sf(color = "red") +
  theme_light() +
  scale_fill_grey() +
...
ggplot() +
  geom_sf(data = usa, aes(fill = GID_1), alpha = .5) +
  geom_sf() +
  theme_light() +
  scale_fill_grey() +
  theme(legend.position = "none")
#can add color in that second geom_sf
