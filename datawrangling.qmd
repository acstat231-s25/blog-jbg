...
#Getting just the hometowns and artist
spotifydataHT <- spotifydata|>
  select(track_artist, hometown)|>
  distinct(track_artist, .keep_all = TRUE)

#grouping and counting how many artist are in each state and delete everything after comma
...
county_map <- maps::map("county", regions = "."
                    , plot = FALSE, fill = TRUE) |>
st_as_sf()
