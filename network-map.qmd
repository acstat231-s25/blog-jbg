...
Bar_Plot1<- 
ggplot(popularity, aes(x=track_artist, y = Count)) +
  geom_bar(color= genre) +
  labs(x="Duration of movie (minutes)"
       , y="Number of movies"
       , fill="") +
  theme_classic()
...
#Plot: 
Plot1<- ggraph(artistcollab_network_Sliced) +
...
#| label: Network Map: Eigenvector Centrality 
#| warning: false
#| message: false
#| out-width: 100%
#| fig-height: 10
...
# Create the Nodes and Edges for a visble Graph 
artistcollab_networkE <- artists |>
      select(track_artist, collabArtist, genre) |>
      as_tbl_graph(directed=FALSE)

gorder(artistcollab_networkE) #Number of Nodes 
gsize(artistcollab_networkE) # Number of Edges 

# Transfer genre info to nodes for ggplot to read it correctly 
artistcollab_networkE <- artistcollab_networkE |>
  activate(nodes) |>
  left_join(
    artists |> 
      select(name = track_artist, genre, collabArtist) |> 
...
Eig_artistcollab_network<- artistcollab_networkE|>
  activate(nodes)|>
